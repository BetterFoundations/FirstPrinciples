...
**S10 · `http-client`** — fresh chat, Claude Sonnet 5 · `/effort high`. Story: `PKG-HTTP`. Not Cursor-delegated: the abort-signal composition and network-vs-HTTP error distinction are subtly wrong in ways that still look like they work. Per its own prompt: read `EXECUTION-CHECKLIST.md` (especially the S7 core-layering decision) and `../Preparation_Docs/typed-fetch-client-spec.md` IN FULL — that spec is authoritative and wins over the ecosystem spec's §10.3 wherever they differ; do not read the ecosystem spec this session. Build `@firstprinciples/http-client`, target 3–5KB gzipped: client factory, get/post/put/patch/delete, endpoint-definition pattern, retry with exponential backoff + jitter, timeout via `AbortController`, `onRequest`/`onResponse` hooks, pluggable validation adapter. Four things called out as easy to get subtly wrong: (1) a network failure must be a distinct result variant from a typed HTTP error response; (2) a caller-supplied `AbortSignal` must be *combined* with the internal timeout controller (whichever fires first wins), not replace it; (3) default retry policy is network errors and 5xx only, never 4xx, verified with fake timers; (4) the validation adapter is a pluggable slot, never a package dependency. Error variants carry `core` error instances per the S7 `Result`/error layering decision — build `ApiResult` exactly as that decision's write-up specifies (`Ok<T> & { status }` / `Err<E> & { status, kind }`), and promote `NetworkError` into `core` per the open item above rather than defining it locally. Verify tree-shaking (importing only `createApiClient` must not pull in unused code) and confirm the bundle actually hits 3–5KB before claiming it anywhere. Full DoD before finishing; update this file; open a PR.
